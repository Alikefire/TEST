...
class CustomDPOTrainer(DPOTrainer):
    ...
    def compute_preference_loss(
        ...
        if not self.finetuning_args.use_ref_model:
            ...
            if self.loss_type == "nca_pair":#新增nca_pair逻辑
    ...
    def concatenated_forward(
        ...
        #这里360-llamafactory return最后传的参数的是chosen_length而不是chosen_logps / chosen_length，后续计算有用
        if self.loss_type in ["ipo", "orpo", "simpo"]:
            return chosen_logps, rejected_logps, chosen_logits, rejected_logits, chosen_length * chosen_length
        else:
    ...
    def get_batch_loss_metrics(
        ...
        (
            ...
            policy_chosen_length, #先算了一个policy_chosen_length，再算 policy_chosen_logps_avg
        ) = self.concatenated_forward(model, batch)

        reference_chosen_logps, reference_rejected_logps = self.compute_reference_log_probs(model, batch)

        # NOTE:新增逻辑， correct logits reduction if necessary. Now we only reduce logps
