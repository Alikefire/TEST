...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def training_step(self, model, inputs, *args, **kwargs):
        ...
        if not self._has_dummy_forwarded and getattr(model, "sequence_parallel_group", None) and model.sequence_parallel_group is not None:
            model.eval()
            with torch.no_grad():
                _ = model(**inputs)
            model.train()
    ...
    def _get_train_sampler(self):
        #添加sequence_parallel_group 的or判断逻辑
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        """
        if getattr(model, "sequence_parallel_group", None) is None:  # no sequence parallel, compute as it is
        ...
        #     # other transformer version should not scale the loss
        #     if return_outputs:
