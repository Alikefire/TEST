...
class DataArguments:
    ...
    # 在 DataArguments 类中添加mft相关参数:
    use_mft: bool = field(
        default=False, metadata={"help": "Whether to use Masked Fine-Tuning."}
    )
    mft_initial_mask_probability: float = field(
        default=0.0, metadata={"help": "Initial mask probability for MFT warmup."}
    )
    mft_target_mask_probability: float = field(
        default=0.20, metadata={"help": "Target mask probability for MFT."}
    )
    mft_warmup_ratio: float = field(
        default=0.0, metadata={"help": "Ratio of total training steps for MFT warmup. 0 means no warmup."}
    )
    mft_mask_token_strategy: str = field(
        default="pad_token", metadata={"help": "Token to use for masking ('pad_token' or other special token)."}
    )
