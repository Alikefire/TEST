...
class S2L(Schedule):
    ...
    def faiss_kmeans_selection(self, features, n):
        ...
        I = I.flatten() # 将 I 展平为一维数组
        
        print(f"*** Rank = {torch.distributed.get_rank()}, **K-means took {time.time() - start_time} seconds")
        
        # --- 开始可视化代码 ---
        if torch.distributed.get_rank() == 0: # 只在 rank 0 进程绘图和保存
            num_clusters = kmeans.k
            # 创建一个图形和一组子图，每个聚类一个子图，或者在一张图上用不同颜色
            # 示例：为每个聚类创建一个单独的图
            # 或者，更常见的是在一张图上用不同颜色表示不同聚类
            plt.figure(figsize=(12, 8))
            
            # 定义一些颜色，或者使用 colormap
            colors = plt.cm.get_cmap('viridis', num_clusters)

            for cluster_id in range(num_clusters):
                cluster_member_indices = np.where(I == cluster_id)[0]
                if len(cluster_member_indices) == 0:
                    continue
                
                # 绘制该聚类中每个成员的 loss 轨迹
                for member_idx in cluster_member_indices:
                    loss_trajectory = features[member_idx].numpy()
                    plt.plot(loss_trajectory, color=colors(cluster_id), alpha=0.5) # 使用带透明度的颜色
            
            plt.title(f'Loss Trajectories by Cluster (k={num_clusters})')
            plt.xlabel('Checkpoint/Epoch Index')
            plt.ylabel('Loss')
            # 如果需要图例，可以为每个聚类创建一个代理艺术家
            # handles = [plt.Line2D([0], [0], color=colors(i), lw=2) for i in range(num_clusters)]
            # labels = [f'Cluster {i}' for i in range(num_clusters)]
            # plt.legend(handles, labels, title="Clusters")
            plt.grid(True)
            
            # 保存图像
            plot_save_path = os.path.join(self.args.get("output_dir", "."), "loss_trajectories_by_cluster.png")
