...
def sample_and_split_data(base_data_path, output_base_path):
    ...
    # Create a general output path if it doesn't exist
    ...
    for subset_name in os.listdir(base_data_path):
        ...
        # Create a directory for the current subset's outputs
        subset_output_path = os.path.join(output_base_path, subset_name)
        os.makedirs(subset_output_path, exist_ok=True)
        
        # Create a directory for the current subset's training data (per cluster)
        subset_train_output_path_dir = os.path.join(subset_output_path, "train_clusters")
        os.makedirs(subset_train_output_path_dir, exist_ok=True)
        ...
        for filename in os.listdir(subset_path):
            ...
            # Save training data for the current cluster
            if train_data_cluster:
                cluster_train_output_filename = f"{original_filename_no_ext}_train.json"
                cluster_train_output_path = os.path.join(subset_train_output_path_dir, cluster_train_output_filename)
                try:
        ...
        # Save aggregated validation data for the current subset
        if subset_all_validation_data:
            subset_validation_output_path = os.path.join(subset_output_path, f"{subset_name}_validation.json")
            try:
        ...
        # Save aggregated evaluation data for the current subset
        if subset_all_evaluation_data:
            subset_evaluation_output_path = os.path.join(subset_output_path, f"{subset_name}_evaluation.json")
            try:
...
if __name__ == '__main__':
    ...
    input_split_data_directory = '/home/xiexin/xx_help/LLaMA-Factory/output_per_dataset_analysis/split_data'
    # This will create a new directory 'processed_splits' inside 'output_per_dataset_analysis'
    # or use any other path you prefer for the output.
    output_directory = '/home/xiexin/xx_help/LLaMA-Factory/output_per_dataset_analysis/processed_splits'
