...
def _get_preprocessed_dataset(
    ...
    if not data_args.streaming:
        kwargs = dict(
            num_proc=data_args.preprocessing_num_workers,
            load_from_cache_file=(not data_args.overwrite_cache) or (training_args.local_process_index != 0),#local_process_index表示的是进程索引号，如果是分布式进程的非主进程则需要开启load_from_cache_file
...
def _get_sequence_parallel_dataset(
    ...
    if not isinstance(dataset, IterableDataset):
        ...
        padded_dataset = dataset.map(
            pad_sequence_func, batched=True, batch_size=data_args.preprocessing_batch_size
        )
    kwargs = dict(
        num_proc=data_args.preprocessing_num_workers,
        load_from_cache_file=(not data_args.overwrite_cache) or (training_args.local_process_index != 0),
    ...
    if not isinstance(dataset, IterableDataset):
        ...
        sp_dataset = padded_dataset.map(
            sp_dataset_func, batched=True, batch_size=data_args.preprocessing_batch_size
        )
    return sp_dataset
...
def get_dataset(
    ...
    with training_args.main_process_first(desc="pre-process dataset"):
        ...
        #新增数据处理逻辑，使用_get_sequence_parallel_dataset处理数据
