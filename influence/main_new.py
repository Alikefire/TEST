...
from cal_KFAC import cal_ihvp, cal_grad_new, cal_influence, cal_influence_dwi
import warnings
import datetime
import pandas as pd
...
def setup_distributed():
    torch.distributed.init_process_group(backend="nccl", timeout=datetime.timedelta(seconds=180000)) #backend="gloo"表示只使用CPU计算
...
def main(args):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=64, num_workers=64, drop_last=True)
    ...
    per_val_list = []
    per_val_cluster_list = []  # 新增：专门存储验证聚类的梯度路径
    ...
    validation_dataloader = DataLoader(validation_dataset, sampler=validation_sampler, batch_size=64, num_workers=64)
    # validation_grad_path = cal_grad_new(validation_dataloader, model, args.save_path + f"/val_avg_grad", args, local_rank) #注释此行以避免重复计算
    ...
    for subset in list_subdirectories(args.validation_path):
        ...
        if os.path.exists(subset_path):
            ...
            dataloader = DataLoader(dataset, sampler=sampler, batch_size=64, num_workers=64)
            subset_grad_path = cal_grad_new(dataloader, model, args.save_path + f"/val_{subset}_grad", args, local_rank) #注释此行以避免重复计算
    ...
    # # 新增：处理验证集聚类（每个聚类分别计算grad.pt）
    # for i, subset in enumerate(args.sub_train):
    #     subset_path = os.path.join((args.validation_path).replace("validation", "validation_cluster"), subset)
    #     if os.path.exists(subset_path):
    ...
    #                 grad_save_path = args.save_path + f"/val_{subset}_{file_basename}_grad"
    #                 avg_grad_path = cal_grad_new(dataloader, model, grad_save_path, args, local_rank) #注释此行以避免重复计算
    ...
    #         # 将该子集的所有文件梯度路径添加到列表中
    #         per_val_cluster_list.extend(subset_grad_paths)
    
    if local_rank == 0:
        print(per_val_list)
        print(per_val_cluster_list)
    ...
    for i in range(len(per_train_list)):
        for j in range(len(per_val_list)):
            influence_list = cal_influence_dwi(hessian_path = args.save_path,
    ...
    if local_rank == 0:
        ...
        df.to_csv(args.save_path + f"/influence_dwi.csv", index=True, header=True)
    
    # 计算聚类影响力矩阵（训练子集 vs 验证聚类）
    if len(per_val_cluster_list) > 0:
        cluster_result = np.zeros((len(per_val_cluster_list), len(per_train_list)))
        
        for i in range(len(per_train_list)):
            for j in range(len(per_val_cluster_list)):
                influence_list = cal_influence(hessian_path = args.save_path,
                            train_grad_path = per_train_list[i],
                            validation_grad_path = per_val_cluster_list[j], 
                            local_rank = local_rank)
                total_sum = sum(influence_list)
                cluster_result[j, i] = total_sum.item()
        ...
        if local_rank == 0:
            print("Cluster influence matrix:")
            print(cluster_result)
            df_cluster = pd.DataFrame(cluster_result, index=per_val_cluster_list, columns=per_train_list)
            df_cluster.to_csv(args.save_path + f"/influence_cluster.csv", index=True, header=True)
    else:
        if local_rank == 0:
            print("No validation clusters found, skipping cluster influence calculation.")
